...
then
    echo "L2_TOKEN_ADDR_LIDO variable inside .env file is not set."
    echo "Deploying 'Wrapped Lido Ether Token' on L2..."
    ...
    DeploymentJSON=$(cast send --rpc-url "http://localhost:8546" "0x4200000000000000000000000000000000000012" "createOptimismMintableERC20WithDecimals(address,string,string,uint8)" $L1_TOKEN_ADDR_LIDO "Wrapped Lido Local Token" wlstETH 18 --private-key $PRIVATE_KEY)
    L2_TOKEN_ADDR_LIDO="0x$( echo $DeploymentJSON  | awk 'BEGIN{FS="topics:*"}{print $2}' |  awk -F, '{print $3}' |awk 'BEGIN{FS="\]*"}{print $1}' | cut -d "\"" -f 2 | tail -c 41)"
    echo "L2_TOKEN_ADDR_LIDO deployed to" $L2_TOKEN_ADDR_LIDO
else
    echo "Using L2_TOKEN_ADDR_LIDO deployed at" $L2_TOKEN_ADDR_LIDO
fi

echo "Running E2E tests for Lido contract..."
forge script -vv --rpc-url "http://localhost:8545" script/swap_and_bridge/contracts/SwapAndBridge_e2e_test.s.sol:TestE2EL1Script $L1_LIDO_BRIDGE_ADDR $L1_TOKEN_ADDR_LIDO $L2_LIDO_BRIDGE_ADDR $L2_TOKEN_ADDR_LIDO --sig 'runLido(address,address,address,address)'
forge script -vv --rpc-url "http://localhost:8546" script/swap_and_bridge/contracts/SwapAndBridge_e2e_test.s.sol:TestE2EL2Script $L2_TOKEN_ADDR_LIDO --sig 'run(address)'
echo "Done."
